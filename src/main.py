...
APP_STATE = {"total_revenue": 0.0, "processed_orders": []}
...
def format_domestic_address(user_info_dict):
    street = user_info_dict["street"]
    city = user_info_dict["city"]
    state = user_info_dict["state"]
    zip_code = user_info_dict["zip"]
...
def process_checkout(user_id, cart_items, cc_number, cvv):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS orders (id INTEGER PRIMARY KEY, user_id INTEGER, total REAL, date TEXT)"
    )
    ...
    if payment_status is True:
        ...
        APP_STATE["total_revenue"] += total
        APP_STATE["processed_orders"].append(user_id)
