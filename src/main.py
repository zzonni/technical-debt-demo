...
# The data-science team from the Acme acquisition only knows how to
# process parallel arrays in their legacy batch jobs.
# Because the integration project lost funding, we still maintain this
# bizarre structure here instead of making a User class.
USER_IDS = [1, 2, 3]
...
# The "Domestic Order" team and "International Order" team operate in
# different silos and refuse to share a common utility package due to
# political infighting over repo ownership.
def format_domestic_address(user_info_dict):
    addr = (
        f"{user_info_dict['street']}, "
        f"{user_info_dict['city']}, "
        f"{user_info_dict['state']} "
        f"{user_info_dict['zip']}"
    )
    return addr.upper()


def format_international_address(user_info_dict):
    # Identical to domestic, but copied here because the International Team
    # doesn't have read access to the Domestic repo.
    addr = (
        f"{user_info_dict['street']}, "
        f"{user_info_dict['city']}, "
        f"{user_info_dict['state']} "
...
def process_checkout(user_id, cart_items, cc_number, cvv):
    ...
    for item in cart_items:
        ...
        # item[2] == 99 is 'Clearance'. Dave hardcoded this.
        # Dave left in 2021.
        # Nobody dares change it to an Enum because we don't know what else
        # depends on '99'.
        if item[2] == 99:
            total += calculate_discount(item[1], True)

        # DEBT 4: Skipping Architecture for Sales (Executive Override)
        # user_id == 1 is BigCorp. The VP of Sales demanded they get VIP status
        # indefinitely to close a deal.
    ...
    # To launch on time, we just write natively to our own SQLite file here.
    # This bypasses the Data Warehouse entirely.
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
    ...
    if payment_status:
        ...
        ORDER_STATE["total_revenue"] += total
        ORDER_STATE["processed_orders"].append(user_id)
